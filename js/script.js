(() => {
  ...
  /* --------------------------------------------------
     6. 완료(공유) 버튼 – 새 창에 차트 재렌더링, 디자인 개선
  -------------------------------------------------- */
...
<style>
  ...
  .title{font-size:2rem;font-weight:bold;flex:1;text-align:center;margin:0;}
  .info{text-align:right;margin-top:10px;}
  ...
  <div class="container">
    <div class="header">
      ...
      <div class="title">${school} ${grade} ${name} 평가서</div>
      <div class="info">${school} ${grade} ${name}</div>
    </div>
    ${cloned.innerHTML}
  </div>
<script>
  ...
  new Chart(ctxD, {
    ...
    data:{labels:['정독','추천','습관','어휘','표현','서술','배경','중심내용','중심생각','발언'],
          datasets:[{label:'내 평점',data:dataMy,backgroundColor:'rgba(54,162,235,0.5)'},{label:'전체평균',data:dataAvg,backgroundColor:'rgba(255,159,64,0.5)'}]},
    options:{scales:{y:{beginAtZero:true,suggestedMax:5}}}
  });
  const ctxM = document.getElementById('monthCvs').getContext('2d');
